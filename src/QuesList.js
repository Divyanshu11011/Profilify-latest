...
useEffect(() => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await fetch('https://profilify-latest.onrender.com/generateQuestions', {
        method: 'POST',
        headers: {
  ...
  const fetchTableNames = async () => {
    try {
      const response = await fetch('https://profilify-latest.onrender.com/getTableNames');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
  ...
  const submitResultsToServer = async () => {
    try {
      if (finalResultsMap && questions.length > 0 && !isResultsSubmitted) {
        ...
        const response = await fetch('https://profilify-latest.onrender.com/submitResults', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
